...
class TransactionRepository {
  ...
  Future<List<AppTransaction>> getByUser(String userId, {int limit = 50}) => readService.fetchByUser(userId, limit: limit);
  Future<int?> countByUser(String userId) => readService.countByUser(userId);
  Stream<List<AppTransaction>> watchByUser(String userId) => readService.watchByUser(userId);
  Future<List<AppTransaction>> listAll({int limit = 100}) => readService.fetchAll(limit: limit);

  // Write facade (return null on success, or String error)
  Future<String?> create(AppTransaction tx) => writeService.createTransaction(tx: tx);
  Future<String?> update(String docId, Map<String, dynamic> data) => writeService.updateTransaction(docId: docId, data: data);
